...
FT_BEGIN_HEADER
   ...
   *
   * @description:
   *   A handle to a FreeType lib instance.  Each 'lib' is completely
   ...
   *   @FT_Open_Face), the lib looks for a Unicode charmap within the
...
#define FT_STYLE_FLAG_BOLD    ( 1 << 1 )
   ...
   *
   *   generic ::
   *     A typeless pointer, unused by the FreeType lib or any of its
   ...
   * @fields:
   *   lib ::
   *     A handle to the FreeType lib instance this slot belongs to.
   *
   *   face ::
   *     A handle to the parent face object.
   *
   ...
   *
   * @description:
   *   Initialize a new FreeType lib object.  The set of modules that are
   *   registered by this function is determined at build time.
   *
   * @output:
   *   alibrary ::
   *     A handle to a new lib object.
   *
   * @return:
   ...
   *
   * @description:
   *   Destroy a given FreeType lib object and all of its children,
   *   including resources, drivers, faces, sizes, etc.
   *
   * @input:
   *   lib ::
   *     A handle to the target lib object.
   *
   * @return:
   *   FreeType error code.  0~means success.
   */
...
#define ft_open_params    FT_OPEN_PARAMS
   ...
   *     {
   *       ...
   *       error = FT_Open_Face( lib, args, i, &face );
   *       ...
   *       FT_Done_Face( face );
   *       ...
   *     }
   ...
   *
   *
   *       error = FT_Open_Face( lib, args, id, &face );
   *       if ( error ) { ... }
   *
   *       num_faces     = face->num_faces;
...
#define ft_kerning_unscaled  FT_KERNING_UNSCALED
   ...
   *
   * @description:
   *   Set or override certain (lib or module-wide) properties on a
   ...
   *   option and use the lib or module default again.
   *
   * @input:
   *   face ::
   *     A handle to the source face object.
...
#define FT_FSTYPE_BITMAP_EMBEDDING_ONLY         0x0200
   ...
   *
   * @note:
   *   The version number of FreeType if built as a dynamic link lib with
...
#define FREETYPE_PATCH  2
   ...
   *
   * @description:
   *   Return the version of the FreeType lib being used.  This is useful
   *   when dynamically linking to the lib, since one cannot use the
   ...
   *
   * @note:
   *   The reason why this function takes a `lib` argument is because
   *   certain programs implement lib initialization in a custom way that
   *   doesn't use @FT_Init_FreeType.
   *
   *   In such cases, the lib version might not be available before the
   *   lib object has been created.
   */
  FT_EXPORT( void )
