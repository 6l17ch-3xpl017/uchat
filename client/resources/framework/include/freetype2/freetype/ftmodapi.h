...
#define ft_module_driver_hints_lightly  FT_MODULE_DRIVER_HINTS_LIGHTLY
   ...
   *
   * @description:
   *   Add a new module to a given lib instance.
   *
   * @inout:
   *   lib ::
   *     A handle to the lib object.
   *
   * @input:
   *   clazz ::
   ...
   *
   * @description:
   *   Remove a given module from a lib instance.
   *
   * @inout:
   *   lib ::
   *     A handle to a lib object.
   *
   * @input:
   *   module ::
   ...
   * @input:
   *    lib ::
   *      A handle to the lib the module is part of.
   *
   *    module_name ::
   *      The module name.
   *
   ...
   *
   *      bar = 1;
   *      FT_Property_Set( lib, "foo", "bar", &bar );
   *    ```
   *
   ...
   *
   *
   *      FT_Property_Get( lib, "foo", "baz", &baz );
   *    ```
   *
   ...
   *   then only destroys a lib if the counter is~1, otherwise it simply
   ...
   * @input:
   *   lib ::
   *     A handle to a target lib object.
   *
   * @return:
   *   FreeType error code.  0~means success.
   *
   ...
   *
   * @description:
   *   This function is used to create a new FreeType lib instance from a
   ...
   *
   *   Don't use @FT_Done_FreeType but @FT_Done_Library to destroy a lib
   *   instance.
   *
   * @input:
   *   memory ::
   ...
   * @output:
   *   alibrary ::
   *     A pointer to handle of a new lib object.
   *
   * @return:
   *   FreeType error code.  0~means success.
   *
   ...
   *
   * @description:
   *   Discard a given lib object.  This closes all drivers and discards
   *   all resource objects.
   *
   * @input:
   *   lib ::
   *     A handle to the target lib.
   *
   * @return:
   *   FreeType error code.  0~means success.
   *
...
#define FT_DEBUG_HOOK_TRUETYPE  0
   ...
   *
   * @description:
   *   Add the set of default drivers to a given lib object.  This is
   *   only useful when you create a lib object with @FT_New_Library
   ...
   *   bytecode support compiled in this version of the lib.
   *
   */
   ...
   *      The lib doesn't implement any kind of bytecode interpreter.
   *
   *    FT_TRUETYPE_ENGINE_TYPE_UNPATENTED ::
   *      Deprecated and removed.
   *
   *    FT_TRUETYPE_ENGINE_TYPE_PATENTED ::
   *      The lib implements a bytecode interpreter that covers the full
   ...
   *    TrueType virtual machine a given lib instance supports.
   *
   * @input:
   *    lib ::
   *      A lib instance.
