...
static int get_len_of_request(t_message *Message) {
    ...
    result += get_len_of_item(mx_itoa(Message->time));
    result += get_len_of_item(mx_itoa(Message->changed));
...
int add_message_to_db(t_message *Message) {
    ...
    concatenate_with_request(&request, mx_itoa(Message->time));
    concatenate_with_request(&request, mx_itoa(Message->changed));
