...
#define FT_CFF_HINTING_ADOBE     FT_HINTING_ADOBE
   ...
   *   rendering lib implement a counter-measure: stem darkening at
   ...
   *   fundamentally different lib with different internals.
   *
   */
...
#define TT_INTERPRETER_VERSION_40  40
   ...
   *   quite complicated and thus needs a separate lib on top of
   *   FreeType.
   *
   ...
   *
   *     FT_Init_FreeType( &lib );
   *     FT_New_Face( lib, "foo.ttf", 0, &face );
   *
   *     prop.face = face;
   *
   *     FT_Property_Get( lib, "autofitter",
...
#define FT_AUTOHINTER_SCRIPT_INDIC  3
   ...
   *     FT_Init_FreeType( &lib );
   *
   *     FT_Property_Set( lib, "autofitter",
   ...
   *   the HarfBuzz lib access OpenType features for getting better glyph
   ...
   *     FT_Init_FreeType( &lib );
   *
   *     FT_Property_Set( lib, "autofitter", "warping", &warping );
   *   ```
   *
   * @since:
   *   2.6
