...
FT_BEGIN_HEADER
   ...
   * @input:
   *   lib ::
   *     A handle to the lib object from where the outline is allocated.
   *     Note however that the new outline will **not** necessarily be
   *     **freed**, when destroying the lib, by @FT_Done_FreeType.
   *
   *   numPoints ::
   ...
   *
   * @note:
   *   The reason why this function takes a `lib` parameter is simply to
   *   use the lib's memory allocator.
   */
  FT_EXPORT( FT_Error )
   ...
   * @input:
   *   lib ::
   *     A handle of the lib object used to allocate the outline.
   *
   *   outline ::
   ...
   * @input:
   *   lib ::
   *     A handle to a FreeType lib object.
   *
   *   outline ::
   *     A pointer to the source outline descriptor.
   *
