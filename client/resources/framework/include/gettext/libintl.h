...
/* We redirect the functions to those prefixed with "libintl_".  This is
   ...
   lib (namely, Solaris 2.4 and newer, and GNU libc 2.0 and newer).
   If we used the unprefixed names, there would be cases where the
   definition in the C lib would override the one in the libintl.so
   shared lib.  Recall that on ELF systems, the symbols are looked
   up in the following order:
     ...
     4. in the dlopen()ed shared libraries, in the order in which they were
        dlopen()ed.
   The definition in the C lib would override the one in libintl.so if
   either
     * -lc is given on the link command line and -lintl isn't, or
     * -lc is given on the link command line before -lintl, or
     * libintl.so is a dependency of a dlopen()ed shared lib but not
